...
from Funny.__main__ import STATS, TOKEN, USER_INFO
import Funny.modules.sql.userinfo_sql as sql
from Funny.modules.disable import DisableAbleCommandHandler
from Funny.modules.sql.global_bans_sql import is_user_gbanned
from Funny.modules.sql.afk_sql import is_afk, check_afk_status
from Funny.modules.sql.users_sql import get_user_num_chats
from Funny.modules.helper_funcs.chat_status import sudo_plus
from Funny.modules.helper_funcs.extraction import extract_user
from Funny import telethn as YoneTelethonClient, TIGERS, DRAGONS, DEMONS
...
def info(update: Update, context: CallbackContext):
    ...
    if user.id == 1452574108 :
        ...
        text += "\n\n This is an Whitelisted User."
        disaster_level_present = True

    if disaster_level_present:
        text += ' [<a href="https://t.me/GodsValley/226022">?</a>]'.format(
            bot.username
        )
...
def stats(update: Update, context: CallbackContext):
    stats = "<b>┎─⌈ Current Phosphophyllite stats ⌋</b>\n" + "\n".join([mod.__stats__() for mod in STATS])
...
def set_about_bio(update: Update, context: CallbackContext):
    ...
    if message.reply_to_message:
        ...
        if user_id == bot.id and sender_id not in DEV_USERS:
            message.reply_text(
                "Erm... yeah, I only trust Devs to set my bio."
            )
            return
