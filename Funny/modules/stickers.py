    ...
    if msg.reply_to_message:
        ...
        if not is_animated:
            try:
                ...
                msg.reply_text("I only supports images.")
                print(e)
                return
                ...
                if e.message == "Stickerset_invalid":
                    ...
                    msg.reply_text(
                        "Sticker is stealed to [pack](t.me/addstickers/%s)"
                        % packname + "\n"
                        "Emoji is:" + " " + sticker_emoji,
                        parse_mode=ParseMode.MARKDOWN,
                    )
    ...
    try:
        ...
        if png_sticker:
            success = context.bot.create_new_sticker_set(
                user.id,
                packname,
                f"{name}`Precious pack" + extra_version,
        ...
        if tgs_sticker:
            success = context.bot.create_new_sticker_set(
                user.id,
                packname,
                f"{name}`s animated sticker pack" + extra_version,
...
KANG_HANDLER = DisableAbleCommandHandler(("steal", "kang"), kang, admin_ok=True)
