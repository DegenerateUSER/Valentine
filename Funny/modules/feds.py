...
import Funny.modules.sql.feds_sql as sql
from Funny import (
    EVENT_LOGS,
    DEV_USERS,
    LOGGER,
...
from Funny.modules.helper_funcs.chat_status import is_user_admin
from Funny.modules.helper_funcs.extraction import (
    extract_unt_fedban,
    extract_user,
    extract_user_fban,
)
from Funny.modules.helper_funcs.string_handling import markdown_parser
...
def del_fed(update: Update, context: CallbackContext):
    ...
    if args:
        ...
        if getinfo is False:
            ...
            return
        if int(getinfo["owner"]) == int(user.id) or int(user.id) in DEV_USERS:
            fed_id = is_fed_id
        else:
...
def is_user_fed_admin(fed_id, user_id):
    ...
    if fed_admins is False:
        return False
    if int(user_id) in fed_admins or int(user_id) in DEV_USERS:
        return True
    else:
        return False
...
def is_user_fed_owner(fed_id, user_id):
    ...
    if str(user_id) == getfedowner or int(user_id) in DEV_USERS:
        return True
    else:
        return False
