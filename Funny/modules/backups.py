...
import Funny.modules.sql.rules_sql as rulessql

# from Yumeko.modules.sql import warns_sql as warnssql
import Funny.modules.sql.blacklist_sql as blacklistsql
from Funny.modules.sql import disable_sql as disabledsql

# from Yumeko.modules.sql import cust_filters_sql as filtersql
# import Yumeko.modules.sql.welcome_sql as welcsql
import Funny.modules.sql.locks_sql as locksql
...
def export_data(update, context):
    ...
    with open("lunaBot{}.backup".format(chat_id), "w") as f:
    ...
    context.bot.sendDocument(
        current_chat_id,
        document=open("lunaBot{}.backup".format(chat_id), "rb"),
        caption="💾*Successfully Exported backup:*\nChat: `{}`\nChat ID: `{}`\nOn: `{}`\n\nNote: This `lunaBot-Backup` was specially made for notes 📚.".format(
            chat.title, chat_id, tgl
        ),
        timeout=360,
    ...
    os.remove("lunaBot{}.backup".format(chat_id))  # Cleaning file
